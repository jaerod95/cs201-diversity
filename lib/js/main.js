            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < 7; i++) {
                        ...
                        ddd.innerHTML = "<div style='background: " + temp.style.background + "; width: 20px; height: 20px; margin-right: 10px; margin-top: 15px;'></div>"
                        var p = document.createElement('p');
                        p.innerHTML = load.race[i] + ': ' + (percentage * 100).toFixed(2) + "% (" + (data[1][i] * sum).toFixed(0) + ")";
                        ddd.innerHTML += "<p>" + p.innerHTML + "</p>";
                        ddd.style.display = 'flex';

                        document.getElementById('races').appendChild(ddd);
